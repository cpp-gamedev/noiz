...
#include <fstream>
	...
	struct BmpHeader {
		...
		BmpHeader(size_t imageSize) {
			sizeOfBitmapFile += imageSize * imageSize; // * 1; //grayscale
		}
...
class Texture2D {
  public:
	...
	explicit Texture2D(int imageSizeFactor, int dataCount) {
		imageSize *= imageSizeFactor;
		heightValues.reserve(dataCount);
	}


	auto add(float const value) -> Texture2D& {
	...
	void writeImage() {
		std::vector<std::vector<uint16_t>> interpolatedValue;
		interpolatedValue.resize(imageSize);
