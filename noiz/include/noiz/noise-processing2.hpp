...
public:
	...
	std::reference_wrapper<Noise2<Type>> noise;
	...
	auto raw_noise(noiz::Vec2<Type> const& point) const -> Type{
		return noise.get().at(point * step);
	}

	auto basic_processing(noiz::Vec2<Type> const& point) const -> Type {
	...
	auto turbulence_processing(noiz::Vec2<Type> const& point) const -> Type {
		Type amplitude = this->amplitude;
		Type frequency = this->frequency;
	...
	auto billowy_processing(noiz::Vec2<Type> const& point) const -> Type {
		return std::abs(noise.get().at(point * step));
	}

	auto rigid_processing(noiz::Vec2<Type> const& point) const -> Type {
		return 1.f - std::abs(noise.get().at(point * step));
	}

	auto hybrid_multi_fractal_processing(noiz::Vec2<Type> const& point) -> Type {
		noiz::Vec2<Type> temp_point = point;
